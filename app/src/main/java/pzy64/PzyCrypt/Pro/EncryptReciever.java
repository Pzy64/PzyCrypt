 ...
 */
public class EncryptReciever extends BroadcastReceiver {
    @Override
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getBooleanExtra("ENC_CANCEL", false)) {
            Snackbar.make(MainActivity.xxx, "Encryption cancelled by user !!", Snackbar.LENGTH_LONG).show();
            MainActivity.cancelled = false;
            MainActivity.cancel.hide();
            MediaPlayer m = MediaPlayer.create(EncryptServiceX.x, R.raw.err);
            m.start();
            MainActivity.working = false;
        }
